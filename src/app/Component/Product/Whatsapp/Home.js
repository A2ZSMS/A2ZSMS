...
// ── TESTING FLAG ──────────────────────────────────────────────
// true  → only TeleCRM fires; Make.com/Web3Forms/gtag + msgmaker are SKIPPED
...
const Home = () => {
  ...
  const handleFinish = async (values) => {
    ...
    try {
      ...
      fireOjivaSMS(trimmedName, mobileDigits);
      if (!SMS_ONLY_TEST) fireTeleCRM(trimmedName, mobileDigits, "", "", "Waba Service", "Lead from Test WhatsApp Widget on /whatsapp-api", extras);
